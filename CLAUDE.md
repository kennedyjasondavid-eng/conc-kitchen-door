...
- **Not fixed here (noted):** Egg Salad Wrap (W1 Wed lunch) carries a stray `hasPork:true` — a vegetarian dish mislabeled, which also routes a phantom Halal section. It's not a pork dish so the invariant guard doesn't apply; correct it in DOOR's menu editor (clear `hasPork`) so the workbook/app-state is right.
