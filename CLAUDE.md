...
- **One publish is 10 commits** (per-file `_ghPushFileNow`), so 9 CI runs per publish evaluate partially-published artifact sets that never existed. Slice D1 ports EXPO's `pushFilesToGitHubAtomic`.
